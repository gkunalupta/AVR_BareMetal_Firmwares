...
//
//    Step 1: enter the input pins into the array RCPin[] = {}. 
//
//            - Any number of pins can be entered into RCPin[] (pins available 0 - 13 and A0 - A5)
//            - The pins do not need to be in numerical order, for example RCPin[] = {A0,5,6,10,8} for 5 channels, or RCPin[] = {A0,5} for 2 channels
...
const int RCPin[]={6,7,8,9,10,11,12,13}; // an array to identify the PWM input pins (the array can be any length) 
                                  // first pin is channel 1, second is channel 2...etc

int RC_inputs = 0;                // The number of pins in RCPin that are connected to an RC receiver. Addition pins not connected to an RC receiver could be used for any other purpose i.e. detecting the echo pulse on an HC-SR04 ultrasonic distance sensor
                                  // When 0, it will automatically update to the number of pins specified in RCPin[] after calling setup_RCRead().                                                
...
const int num_ch = sizeof(RCPin)/sizeof(int);  // calculate the number of input pins (or channels)
...
byte RCPin_port[num_ch];                       // identify which port each input pin belongs to (0 = PORTH, 1 = PORTB, 2 = PORTF,3==PORTK)
...
void RCPin_to_port()
{
  for (int i = 0; i < num_ch; i++){
    ...
    // determine which port and therefore ISR (PCINT0_vect, PCINT1_vect or PCINT2_vect) each RCPin belongs to.
                                                                  RCPin_port[i] = 2;    // pin belongs to PCINT1_vect (PORT K)
    if (RCPin[i] >= 6 && RCPin[i] <= 9)                         RCPin_port[i] = 0;    // pin belongs to PCINT2_vect (PORT H)
    else if (RCPin[i] >= 10 && RCPin[i] <= 13)                   RCPin_port[i] = 1;    // pin belongs to PCINT0_vect (PORT B)
    ...
    if(RCPin[i] == 6 )                         RCPin_reg[i] = 0b00001000;
    else if(RCPin[i] == 7 || RCPin[i] == 10 )  RCPin_reg[i] = 0b00010000;
    else if(RCPin[i] == 8 || RCPin[i] == 11)   RCPin_reg[i] = 0b00100000;
    else if(RCPin[i] == 9 || RCPin[i] == 12 )  RCPin_reg[i] = 0b01000000;
   // else if(RCPin[i] == 4 || RCPin[i] == A4 || RCPin[i] == 12)   RCPin_reg[i] = 0b00010000;
 // else if(RCPin[i] == 5 || RCPin[i] == A5 || RCPin[i] == 13)   RCPin_reg[i] = 0b00100000;
    else if(RCPin[i] == 13)                                         RCPin_reg[i] = 0b10000000;
    //else if(RCPin[i] == 7)                                         RCPin_reg[i] = 0b10000000;
...
void setup_RCRead(){
  ...
  for(int i = 0; i < num_ch; i++){              // run through each input pin
    pciSetup(RCPin[i]);                        // enable pinchange interrupt for pin
  }
  RCPin_to_port();                             // determines the port for each input pin
                                                // RCPin_to_port() also coverts the pin number in RCPin[] (i.e. pin 11 or pin A0) to the pin position in the port register (i.e. 0b00000001) for use in the ISR.
...
{                                                 // this function will run if a pin change is detected on portH
  ...
  for (int i = 0; i < num_ch; i++){                               // run through each of the channels
    if (RCPin_port[i] == 0){                                     // if the current channel belongs to portH
      
      if(prev_pinState[i] == 0 && PINH & RCPin_reg[i]){          // and the pin state has changed from LOW to HIGH (start of pulse)
      ...
      else if (prev_pinState[i] == 1 && !(PINH & RCPin_reg[i])){ // or the pin state has changed from HIGH to LOW (end of pulse)
...
ISR(PCINT1_vect){                                                 // this function will run if a pin change is detected on portB

  pciTime = micros();                                             // Record the time of the PIN change in microseconds

  for (int i = 0; i < num_ch; i++){                               // run through each of the channels
    if (RCPin_port[i] == 2){                                     // if the current channel belongs to portD
      ...
      else if (prev_pinState[i] == 1 && !(PIND & RCPin_reg[i])){ // or the pin state has changed from HIGH to LOW (end of pulse)
        prev_pinState[i] = 0;                                     // record pin state
        PW[i] = pciTime - pwmTimer[i];                            // calculate the duration of the current pulse
        pwmFlag[i] = HIGH;                                        // flag that new data is available
        if(i+1 == RC_inputs) RC_data_rdy = HIGH;
...
void print_RCValue(){                             // display the raw RC Channel PWM Inputs
