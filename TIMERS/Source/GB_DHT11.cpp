...
{	
 ...
 volatile uint8_t gb_cnt = 0;
	...
	for (int gb_z = 0; gb_z < 5; ++gb_z)
	{
        for(int gb_j = 7; gb_j >= 0; --gb_j)
        {
            ...
            { 
				...
				if (TCNT0 <= 170)              // data signal is high for max 71us (as for 0 it is 26-28us and for 1 it is 70us)
				gb_cnt = TCNT0;                   // So for 160 , 71us is generated to waiting for maximum time.
			    else
			  {
            ...
            if ((gb_cnt >= 40) & (gb_cnt <= 70))          //20us to 35 us
            CLEAR_BIT(gb_data[gb_z],gb_j); //decimel0(cnt);printString0("\n");}
			
            else if ((gb_cnt >= 120) & (gb_cnt <= 170))     // 60us to 80us
            SET_BIT(gb_data[gb_z],gb_j);//decimel0(cnt); printString0("\n");}
	...
	if (gb_check != gb_data[4]) return 3;
	/*
	for(int w = 0; w <= 4; w++)
	{ 
	  x[w] = data[w]; 
...
void GB_dht11_init10()
{
	...
	else
	{
		...
		TCNT0=0;
		SET_BIT(DDRB,PB4);
		GB_printString0("Starting dht11\n");
		CLEAR_BIT(PORTB,PB4);
		{TCNT1=0;
		while(TCNT1<=281);}
		SET_BIT(PORTB,PB4);
 ...
 uint8_t  gb_check;
	...
	for (int gb_z = 0; gb_z < 5; ++gb_z)
	{
        for(int gb_j = 7; gb_j >= 0; --gb_j)
        {
            ...
            { CLEAR_BIT(gb_data[gb_z],gb_j); //decimel0(TCNT0);printString0("\n");
			}
            
            else if ((TCNT0 >= 120) & (TCNT0 <= 160))     // 60us to 80us
            { SET_BIT(gb_data[gb_z],gb_j); //printString0("\n");decimel0(TCNT0); printString0("\n");
			}
           
            else
			{GB_printString0("error 2\n"); GB_decimel0(TCNT0); return 2;}
        }
		TCNT0=0;
	}   
	gb_check = (gb_data[0] + gb_data[1] + gb_data[2] + gb_data[3]) & 0xFF;
	...
	if (gb_check != gb_data[4]) return 3;
	//bit0(data[2]);
	for(int gb_w = 0; gb_w < 4; ++gb_w)
	{ gb_arr[gb_w] = gb_data[gb_w]; }
