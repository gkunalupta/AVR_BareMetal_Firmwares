...
ISR(SPI_STC_vect)
{
	...
	RECV_BUFFER_SPI0[RECV_Wr_Index_SPI0]= rx_byte;  
	if(++RECV_Wr_Index_SPI0 > RECV_BUFFER_SIZE_SPI0)
	RECV_Wr_Index_SPI0 = 0;
	if(++RECV_Counter_SPI0 > RECV_BUFFER_SIZE_SPI0)
	{
	
		RECV_Counter_SPI0 = RECV_BUFFER_SIZE_SPI0; 
		RECV_Buffer_Overflow_SPI0 = 1;          
		}                                         
...
void MA_SPI0_init_master (void)
{
     ...
     //	spi_data_direc |= (1<<MOSI)|(1<<SCK);
     spi_control_reg  |= (1<<SPE)|(1<<MSTR)|(1<<SPR0); //Enable SPI, Set as Master  //|(1<<SPR0)
	                                     //Prescaler: Fosc/16, 
	  //LSB First
	  spi_control_reg |= (1<<DORD);
	  
	  
      spi_control_reg &= ~((1<<CPOL) | (1<<CPHA));
...
uint8_t MA_SPI0_exchange_byte (uint8_t data)
{
	//spi_port &= ~(1<<SS);
	//bit0(data);
	//UART_TxChar0('u');
...
void MA_SPI0_send_byte_conti(uint8_t data)
{
	//UART_TxChar0('p');
	spi_data_reg = data;                       //Load data into the buffer
	while(!(SPSR &(1<<SPIF)));
