...
char GB_MA_SPI0_INTRPT_read_byte()
	{
...
void GB_MA_SPI0_INTRPT_read_string()
	{
		...
		{
			gb_RECV_DATA_SPI0[gb_x]= GB_MA_SPI0_INTRPT_read_byte();
			gb_x++;
		}
		GB_printString0(gb_RECV_DATA_SPI0);
		//printString0("\n ");
	}
void GB_MA_SPI0_INTRPT_read_block(char *gb_buff, uint8_t gb_size)
	{
...
void GB_MA_SPI0_init_master (void)
{
	gb_spi_data_direc = (1<<gb_SPI_MOSI)|(1<<gb_SPI_SCK)|(1<<gb_SPI_SS);              //Set MOSI, SCK, SS as Output
	gb_spi_control_reg = (1<<SPE)|(1<<MSTR)|(1<<SPR0); //Enable SPI, Set as Master
	                                     //Prescaler: Fosc/16, Enable Interrupts
}
void GB_MA_slave_select_low(uint8_t gb_SPI_ss)
{
	gb_spi_port &= ~(1<<gb_SPI_ss);
}
void GB_MA_slave_select_high(uint8_t gb_SPI_ss)
{
	gb_spi_port |= (1<<gb_SPI_ss);
}
void GB_MA_SPI0_enable_interrupt()
{
	gb_spi_control_reg |= (1<<SPIE);
}
void GB_MA_SPI0_disable_interrupt()
{
	gb_spi_control_reg &= ~(1<<SPIE);
}
//Function to send and receive data
uint16_t GB_MA_SPI0_exchange_byte (uint16_t gb_data)
{
	//gb_spi_port &= ~(1<<gb_SPI_SS);
	SPDR = gb_data;  // Load data into the buffert until transmission complete
	while(!(SPSR & (1<<SPIF) ));  //Wait until transmission complete
	//gb_spi_port = (0x01 & (1<<gb_SPI_SS));
...
*/
void GB_MA_SPI0_send_byte(uint8_t gb_data)
{
	gb_spi_port &= ~(1<<gb_SPI_SS);
	gb_spi_data_reg = gb_data;                       //Load data into the buffer
	while(!(SPSR &(1<<SPIF)));
	gb_spi_port |= (1<<gb_SPI_SS);
}
void GB_MA_SPI0_send_byte_conti(uint8_t gb_data)
{
	//UART_TxChar0('p');
	gb_spi_data_reg = gb_data;                       //Load data into the buffer
	while(!(SPSR &(1<<SPIF)));
...
*/
void GB_MA_SPI0_send_string(const char * gb_buff)
{
	char gb_c;
	for (const char * gb_p = gb_buff; gb_c = *gb_p; gb_p++)
	{
		GB_MA_SPI0_send_byte(gb_c);
		//UART_TxChar0(a);
	}
}
/*
...
E.g: spi_send_int(12345);
     spi_send_int(9876); 
*/
void GB_MA_SPI0_send_int(uint16_t gb_num)
{
	...
	for(;gb_ptr>=0;--gb_ptr) 
	GB_MA_SPI0_send_byte(gb_buf[gb_ptr]);
}
/*
For sending array of integer format. 
*/
void GB_MA_SPI0_send_intarray(const uint16_t * gb_buff)
{
	char gb_c;
	...
	GB_MA_SPI0_send_byte('\0');

}
/* receives one byte 
E.G: char ch = MA_SPI0_read_byte()
*/
uint8_t GB_MA_SPI0_read_byte()
{    	
	//gb_spi_port &= ~(1<<gb_SPI_SS);
    //uint8_t gb_recv_char=0;
	
	gb_spi_data_reg = NULL;

	while(!(SPSR & (1<<SPIF)));                  //Wait until transmission complete

    return(gb_spi_data_reg);
...
*/
void GB_MA_SPI0_read_block(char gb_block[], uint8_t gb_size)
{
